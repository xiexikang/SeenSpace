...
export function AgentConnectionPage() {
  ...
  async function sendApiTest() {
    ...
    try { const result = await sendAgentRuntimeApi({ apiServer, body }); setApiResponse(result.data); setStatus(`API 调试调用成功 · HTTP ${result.status_code}`) }
  ...
  async function sendMcpTest() {
    ...
    try {
      const result = await sendAgentRuntimeMcp({ mcpServer, body, chatContextId: chatContextId.trim() || undefined, mcpSessionId: mcpSessionId || undefined })
  ...
  return <div className="flex h-dvh overflow-hidden bg-[var(--background)] text-[var(--text-primary)]"><LibrarySidebar /><main className="min-h-0 min-w-0 flex-1 overflow-auto p-6 lg:p-10">
    <div className="mx-auto max-w-[900px]">
      ...
      <section className="mt-5 rounded-[20px] border border-[var(--border)] bg-[var(--panel)] p-5 shadow-sm"><div className="flex items-center gap-2 font-semibold"><Server className="h-4 w-4 text-[var(--accent)]" />API Servers 调试</div><p className="mt-1 text-sm text-[var(--text-secondary)]">选择运行时 API Server 并通过后端发送测试请求。</p><div className="mt-4 grid gap-3 md:grid-cols-2"><label className="text-sm"><span className="mb-1 block text-[var(--text-muted)]">API Server</span><select value={apiServer} onChange={(event) => setApiServer(event.target.value)} className="h-10 w-full rounded-[10px] border border-[var(--border)] bg-[var(--background)] px-3 outline-none focus:border-[var(--accent)]"><option value="">请选择</option>{apiServers.map((name) => <option key={name} value={name}>{name}</option>)}</select></label><label className="text-sm"><span className="mb-1 block text-[var(--text-muted)]">请求 JSON（可选）</span><input value={apiBody} onChange={(event) => setApiBody(event.target.value)} className="h-10 w-full rounded-[10px] border border-[var(--border)] bg-[var(--background)] px-3 font-mono text-sm outline-none focus:border-[var(--accent)]" /></label></div><button onClick={() => void sendApiTest()} disabled={apiLoading} className="mt-3 inline-flex h-10 items-center gap-2 rounded-[10px] bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{apiLoading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}发送 API 测试</button>{apiError ? <div className="mt-3 rounded-[10px] bg-[var(--accent-soft)] px-3 py-2 text-sm text-[var(--accent-strong)]">{apiError}</div> : null}{apiResponse !== null ? <pre className="mt-4 max-h-[360px] overflow-auto rounded-[10px] bg-[var(--panel-soft)] p-3 text-xs leading-5 text-[var(--text-secondary)]">{JSON.stringify(apiResponse, null, 2)}</pre> : null}</section>
      <section className="mt-5 rounded-[20px] border border-[var(--border)] bg-[var(--panel)] p-5 shadow-sm"><div className="flex items-center gap-2 font-semibold"><Server className="h-4 w-4 text-[var(--accent)]" />MCP Servers 调试</div><p className="mt-1 text-sm text-[var(--text-secondary)]">支持 HTTP MCP Server，可发送 JSON-RPC 请求。</p><div className="mt-4 grid gap-3 md:grid-cols-2"><label className="text-sm"><span className="mb-1 block text-[var(--text-muted)]">MCP Server</span><select value={mcpServer} onChange={(event) => { setMcpServer(event.target.value); setMcpSessionId('') }} className="h-10 w-full rounded-[10px] border border-[var(--border)] bg-[var(--background)] px-3 outline-none focus:border-[var(--accent)]"><option value="">请选择</option>{servers.map((name) => <option key={name} value={name}>{name}</option>)}</select></label><label className="text-sm"><span className="mb-1 block text-[var(--text-muted)]">MCP Session ID</span><input value={mcpSessionId} onChange={(event) => setMcpSessionId(event.target.value)} placeholder="initialize 后自动填充" className="h-10 w-full rounded-[10px] border border-[var(--border)] bg-[var(--background)] px-3 font-mono text-sm outline-none focus:border-[var(--accent)]" /></label></div><label className="mt-3 block text-sm"><span className="mb-1 block text-[var(--text-muted)]">JSON-RPC 请求</span><textarea value={mcpBody} onChange={(event) => setMcpBody(event.target.value)} rows={5} className="w-full resize-y rounded-[10px] border border-[var(--border)] bg-[var(--background)] p-3 font-mono text-sm outline-none focus:border-[var(--accent)]" /></label><button onClick={() => void sendMcpTest()} disabled={mcpLoading} className="mt-3 inline-flex h-10 items-center gap-2 rounded-[10px] bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{mcpLoading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}发送 MCP 测试</button>{mcpError ? <div className="mt-3 rounded-[10px] bg-[var(--accent-soft)] px-3 py-2 text-sm text-[var(--accent-strong)]">{mcpError}</div> : null}{mcpResponse !== null ? <pre className="mt-4 max-h-[360px] overflow-auto rounded-[10px] bg-[var(--panel-soft)] p-3 text-xs leading-5 text-[var(--text-secondary)]">{JSON.stringify(mcpResponse, null, 2)}</pre> : null}</section>
    </div>
  </main></div>
}
